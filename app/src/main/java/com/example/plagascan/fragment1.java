    ...
    public void json(String name,InputStream inputStream) throws IOException, JSONException {
        ...
        for (int i = 0; i < plagas.length(); i++) {
            ...
            if (plaga.has("nombre_clase") && plaga.getString("nombre_clase").equals(name)) {
                ...
                cogollero = plaga.getJSONObject("cogollero");
        ...
        if (cogollero != null) {
            nombreCientifico = cogollero.getJSONArray("nombre_cientifico").getString(0);
            scientific.setText("Nombre científico: " + nombreCientifico);
            returnList.clear();
            nombresComunes = cogollero.getJSONArray("nombre_comun");
            returnList = detaildata(nombresComunes);
            resultString = TextUtils.join(" / ", returnList);
            common.setText(resultString);

            imagen = cogollero.getJSONArray("imagenes");
            descripcion = cogollero.getJSONArray("descripcion");
            quimico = cogollero.getJSONArray("control_quimico");
            fuente = cogollero.getString("fuente");
            enlace = cogollero.getString("enlace");
